...
\caption{Chart showing  $p(s \in \mathcal{L} | S \subseteq {L}) $, sorted in increasing order for model data. Human subject data is in blue, model data is in red. $R$ = .75} 
\label{results-chart}
\end{figure} 
...
Observe that the model's performence is effectively all or none. In twelve of the trials, the model's predictions for  $p(s \in \mathcal{L} | S \subseteq {L}) $ was either 0 or 1. This suggests that the posterior is dominated by hypotheses with high likelihoods, all of which predict the same result. In many cases, we observed that the number of hypotheses with posterior probability greater than .1 to be only one or two.  

We note that in cases 16 (S = \{`yyyt', `yt', `yyyt', `yyyt', `yt'\}, s= `yyyyyt'), 17 (S = \{`jojojojo', `jojojojo', `jojojojo'\}, s = `jojo'), and 18 (S = \{`twtwtw', `twtwtw', `twtwtw'\}, s =  `tw'), it is clear that the model weighs a smaller number of transitions greater than human subjects do far more than humans do as a result of our beta term. Here, the prior on the hypothesis is so low that the difference in the likelihood of the base hypothesis as compared to a hypothesis with fewer states and more loops that the posterior probability becomes relatively unlikely. Future work could better determine how this term should be set. However, because this same parameter in part controls for wildcards, lowering it reduces performance on many other cases. 
