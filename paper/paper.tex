...
Observe that the model's performence is effectively all or none. In twelve of the trials, the model's predictions for  $p(s \in \mathcal{L} | S \subseteq {L}) $ was either 0 or 1. After performing inference, the model generally included one or two hypotheses amounting to over 90\% of the voting weight. Thus, though our inference process considered many hypotheses at once, only a very small number had significant impact on the result.

We note that in cases 16 (S = \{`yyyt', `yt', `yyyt', `yyyt', `yt'\}, s= `yyyyyt'), 17 (S = \{`jojojojo', `jojojojo', `jojojojo'\}, s = `jojo'), and 18 (S = \{`twtwtw', `twtwtw', `twtwtw'\}, s =  `tw'), it is clear that the model more strongly prefers the use of the Kleene star. However, because this same parameter in part controls for wildcards, lowering it reduces performance on many other cases. 
